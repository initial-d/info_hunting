...
<div id="head">
	...
	<div id="navigation">
		...
		<br>
		<form   action="tupiansearch"  method="post" >
...
<s:subset source="#onelist" start="#request.star" count="#request.sizes">        
  ...
  <table>
  <tr> 
     <td> <a href="showtupiansearch.jsp?previous=<%=previous%>">上一页</a></td>
      <% for(int i=1;i<pagecount+1;i++){ 
               if(pageno!=i){
       %>
     <td><a href="showtupiansearch.jsp?i=<%=i%>">[第<%=i%>页]</a> </td>   
     <% }else{ %>
               <td>[第<%= i %>页]</td>
           <% } } %>
       
     <td><a href="showtupiansearch.jsp?next=<%=next%>">下一页</a></td>
   </tr>    
  </table>
    </td></tr>  
   </s:else> 
