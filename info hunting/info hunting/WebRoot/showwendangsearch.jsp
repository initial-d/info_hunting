...
<div id="head">
	...
	<div id="navigation">
		...
		<br>
		<form   action="wendangsearch"  method="post" >
...
<s:subset source="#onelist" start="#request.star" count="#request.sizes">        
  ...
  <table>
  <tr> 
     <td> <a href="showwendangsearch.jsp?previous=<%=previous%>">上一页</a></td>
      <% for(int i=1;i<pagecount+1;i++){ 
               if(pageno!=i){
       %>
     <td><a href="showwendangsearch.jsp?i=<%=i%>">[第<%=i%>页]</a> </td>   
     <% }else{ %>
               <td>[第<%= i %>页]</td>
           <% } } %>
       
     <td><a href="showwendangsearch.jsp?next=<%=next%>">下一页</a></td>
   </tr>    
  </table>
    </td></tr>  
   </s:else> 
